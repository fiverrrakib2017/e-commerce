...
@section('content')
      <div class="br-pageheader">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
          ...
          <a class="breadcrumb-item" href="{{route('admin.shop.index')}}">Shop</a>
          <span class="breadcrumb-item active">Pick up Point</span>
        </nav>
      </div><!-- br-pageheader -->
  ...
  <div class="table-wrapper">
    <div class="card">
      <div class="card-header">
        <button  type="button" class="btn btn btn-success"  data-toggle="modal" data-target="#addModal">Add Pickup Point</a>
      </div>
      <div class="card-body">
      ...
      <thead>
        <tr>
          ...
          <th class="">Pickup Point Name</th>
          <th class="">Address</th>
          <th class="">Phone</th>
          <th class="">Pickup Status</th>
          <th class="">Cash On Pickup Status</th>
...
<div id="deleteModal" class="modal fade">
    <div class="modal-dialog modal-dialog-top" role="document">
        ...
        <div class="modal-body tx-center pd-y-20 pd-x-20">
            <form action="{{route('admin.pickup.delete')}}" method="post" enctype="multipart/form-data">
                @csrf
...
<div id="addModal" class="modal fade effect-scale">
        <div class="modal-dialog modal-lg modal-dialog-top mt-4" role="document">
            ...
            <div class="modal-header pd-x-20">
                <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Add Pickup Point</h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
        <!----- Start Add  Form ------->
        <form action="{{route('admin.pickup.store')}}" method="post">
        @csrf
            ...
            <div class="col-xl-12">
                <div class="form-layout form-layout-4">
                    ...
                    <div class="row mb-4 mt-4">
                        <label class="col-sm-3 form-control-label">Staff Name <span class="tx-danger">*</span></label>
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                        <select type="text" name="staff_name" class="form-control"required>
                    ...
                    <div class="row mb-4">
                        <label class="col-sm-3 form-control-label">Pickup Point Name: <span class="tx-danger">*</span></label>
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                        <input type="text" name="pickup_point_name" class="form-control" placeholder="Enter Pickup Point Name" required>
                        </div>
                    </div><!-- row -->
                    ...
                    <div class="row mb-4">
                        <label class="col-sm-3 form-control-label">Pickup Status: <span class="tx-danger">*</span></label>
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                        <select type="text" name="pick_up_status" class="form-control" required>
                            <option value="">---Select---</option>
                    ...
                    <div class="row mb-4">
                        <label class="col-sm-3 form-control-label">Cash On Pickup Status: <span class="tx-danger">*</span></label>
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                        <select type="text" name="cash_on_pickup_status" class="form-control" required>
                            <option value="">---Select---</option>
                            <option value="1">Active</option>
                            <option value="0">inactive</option>
                        </select>
...
<!----- Edit Modal ------->
    <div id="editModal" class="modal fade effect-scale">
        <div class="modal-dialog modal-lg modal-dialog-top mt-4" role="document">
            <div class="modal-content tx-size-sm">
            <div class="modal-header pd-x-20">
                <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Update Pickup Point</h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
        <!----- Start Add  Form ------->
        <form action="{{route('admin.pickup.update')}}" method="post">
        @csrf
            ...
            <div class="col-xl-12">
                <div class="form-layout form-layout-4">
                    ...
                    <div class="row mb-4 mt-4">
                        <label class="col-sm-3 form-control-label">Staff Name <span class="tx-danger">*</span></label>
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                           <input type="text" value="" name="id" class="d-none"> 
                        <select type="text" name="staff_name" class="form-control"required>
                            <option value="">---Select---</option>
                            @foreach ($staff as $item)
                                <option value="{{$item->id}}">{{ $item->name }}</option>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      var table=$("#datatable1").DataTable({
        ...
        ajax: "{{ route('admin.pickup.all_data') }}",
        language: {
    ...
    $('#datatable1 tbody').on('click', '.edit-btn', function () {
      ...
      $.ajax({
          type: 'GET',
          url: "{{ route('admin.pickup.edit', ['id' => ':id']) }}".replace(':id', id),
          success: function (response) {
            console.log(response);
              if (response.success) {
                $('#editModal').modal('show');
                $('#editModal input[name="id"]').val(response.data.id);
                $('#editModal select[name="staff_name"]').val(response.data.staff_id);
                $('#editModal input[name="pickup_point_name"]').val(response.data.name);
                $('#editModal textarea[name="address"]').val(response.data.address);
                $('#editModal input[name="phone"]').val(response.data.phone);
                $('#editModal select[name="pick_up_status"]').val(response.data.pick_up_status);
                $('#editModal select[name="cash_on_pickup_status"]').val(response.data.cash_on_pickup_status);
              } else {
