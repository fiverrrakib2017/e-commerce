  ...
  <div class="table-wrapper">
    <div class="card">
      <div class="card-header">
        <button  type="button" class="btn btn btn-success"  data-toggle="modal" data-target="#addModal">Add New City</a>
      </div>
      <div class="card-body">
...
<div id="deleteModal" class="modal fade">
    <div class="modal-dialog modal-dialog-top" role="document">
        ...
        <div class="modal-body tx-center pd-y-20 pd-x-20">
            <form action="{{route('admin.address.city.delete')}}" method="post" enctype="multipart/form-data">
                @csrf
...
<div id="addModal" class="modal fade effect-scale">
        ...
        <!----- Start Add  Form ------->
        <form action="{{route('admin.address.city.store')}}" method="post">
        @csrf
                ...
                <div class="form-layout form-layout-4">
                     ...
                     <div class="row mb-4">
                        ...
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                          <select type="text" name="state_id" class="form-control"  required>
...
<div id="editModal" class="modal fade effect-scale">
        <div class="modal-dialog modal-lg modal-dialog-top mt-4" role="document">
            <div class="modal-content tx-size-sm">
            <div class="modal-header pd-x-20">
                <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Update City</h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
        <!----- Start Add  Form ------->
        <form action="{{route('admin.address.city.update')}}" method="post">
        @csrf
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      var table=$("#datatable1").DataTable({
        ...
        ajax: "{{ route('admin.address.city.all_data') }}",
        language: {
    ...
    $('#datatable1 tbody').on('click', '.edit-btn', function () {
      ...
      $.ajax({
          type: 'GET',
          url: '/admin/address/city/edit/' + id,
          success: function (response) {
              if (response.success) {
                $('#editModal').modal('show');
                $('#editModal input[name="id"]').val(response.data.id);
                $('#editModal select[name="state_id"]').val(response.data.state_id);
                $('#editModal input[name="city_name"]').val(response.data.name);
