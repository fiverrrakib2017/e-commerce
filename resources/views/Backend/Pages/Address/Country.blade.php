...
@section('content')
      <div class="br-pageheader">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
          ...
          <span class="breadcrumb-item active">All Country</span>
        </nav>
      </div><!-- br-pageheader -->
  ...
  <div class="table-wrapper">
    <div class="card">
      <div class="card-header">
        <button  type="button" class="btn btn btn-success"  data-toggle="modal" data-target="#addModal">Add New Country</a>
      </div>
      <div class="card-body">
...
<div id="deleteModal" class="modal fade">
    <div class="modal-dialog modal-dialog-top" role="document">
        ...
        <div class="modal-body tx-center pd-y-20 pd-x-20">
            <form action="{{route('admin.address.country.delete')}}" method="post" enctype="multipart/form-data">
                @csrf
...
<div id="addModal" class="modal fade effect-scale">
        <div class="modal-dialog modal-lg modal-dialog-top mt-4" role="document">
            ...
            <div class="modal-header pd-x-20">
                <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Add Country</h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
        <!----- Start Add  Form ------->
        <form action="{{route('admin.address.country.store')}}" method="post">
        @csrf
            ...
            <div class="col-xl-12">
                <div class="form-layout form-layout-4">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                        <input type="text" name="country_code" class="form-control" placeholder="Enter Country Code" required>
                        </div>
                    </div><!-- row -->
...
<div id="editModal" class="modal fade effect-scale">
        <div class="modal-dialog modal-lg modal-dialog-top mt-4" role="document">
            <div class="modal-content tx-size-sm">
            <div class="modal-header pd-x-20">
                <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Update Country</h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
        <!----- Start Add  Form ------->
        <form action="{{route('admin.address.country.update')}}" method="post">
        @csrf
        ...
        <div class="modal-body ">
            ...
            <div class="col-xl-12">
                ...
                <div class="row mb-4">
                        <label class="col-sm-3 form-control-label">Country Name: <span class="tx-danger">*</span></label>
                        <div class="col-sm-9 mg-t-10 mg-sm-t-0">
                        <input type="text" name="id" class="form-control d-none" required>
                        <input type="text" name="country_name" class="form-control" placeholder="Enter Country Name" required>
                        </div>
                    </div><!-- row -->
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      var table=$("#datatable1").DataTable({
        ...
        ajax: "{{ route('admin.address.country.all_data') }}",
        language: {
    ...
    $('#datatable1 tbody').on('click', '.edit-btn', function () {
      ...
      $.ajax({
          type: 'GET',
          url: '/admin/address/country/edit/' + id,
          success: function (response) {
              if (response.success) {
                $('#editModal').modal('show');
                $('#editModal input[name="id"]').val(response.data.id);
                $('#editModal input[name="country_name"]').val(response.data.country_name);
                $('#editModal input[name="country_code"]').val(response.data.country_code);
