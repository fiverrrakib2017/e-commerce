  ...
  <div class="table-wrapper">
    <div class="card">
      <div class="card-header">
        <button  type="button" class="btn btn btn-success"  data-toggle="modal" data-target="#addModal">Add New State</a>
      </div>
      <div class="card-body">
...
<div id="deleteModal" class="modal fade">
    <div class="modal-dialog modal-dialog-top" role="document">
        ...
        <div class="modal-body tx-center pd-y-20 pd-x-20">
            <form action="{{route('admin.address.state.delete')}}" method="post" enctype="multipart/form-data">
                @csrf
...
<div id="addModal" class="modal fade effect-scale">
        ...
        <!----- Start Add  Form ------->
        <form action="{{route('admin.address.state.store')}}" method="post">
        @csrf
...
<div id="editModal" class="modal fade effect-scale">
        ...
        <!----- Start Add  Form ------->
        <form action="{{route('admin.address.state.update')}}" method="post">
        @csrf
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      var table=$("#datatable1").DataTable({
        ...
        ajax: "{{ route('admin.address.state.all_data') }}",
        language: {
    ...
    $('#datatable1 tbody').on('click', '.edit-btn', function () {
      ...
      $.ajax({
          type: 'GET',
          url: '/admin/address/state/edit/' + id,
          success: function (response) {
            console.log(response);
              if (response.success) {
                $('#editModal').modal('show');
                $('#editModal input[name="id"]').val(response.data.id);
                $('#editModal input[name="state_name"]').val(response.data.name);
                $('#editModal select[name="country_id"]').val(response.data.country_id);
