...
<div class="br-pageheader">
   <nav class="breadcrumb pd-0 mg-0 tx-12">
      ...
      <a class="breadcrumb-item" href="{{route('admin.supplier.index')}}">Invoice</a>
      ...
      <div class="row d-flex">
         <div class="col-md-7  ">
            <div class="card card-body">
               <form id="form-data" action="{{route('admin.supplier.invoice.update_invoice')}}" method="post">@csrf
                     ...
                     <select type="text" id="supplier_name" name="supplier_id" class="form-control select2" style="width:100%">
                        ...
                        @foreach ($supplier as $supplierItem)
                           @foreach ($data as $invoice)
                                 @if ($invoice->supplier->id == $supplierItem->id)
                                    <option value="{{ $supplierItem->id }}" selected>{{ $supplierItem->fullname }}</option>
                                 @else
                                    <option value="{{ $supplierItem->id }}">{{ $supplierItem->fullname }}</option>
                                 @endif
                           @endforeach
                        @endforeach
                     </select>
                  ...
                  <div class="row">
                     <div class="col-md-12">
                           ...
                           @foreach ($data as $invoice)
                              @foreach ($invoice->items as $item)
                                 <tr>
                                    ...
                                    <td>
                                       <a  type="button" id="itemRow"><i class="icon ion-close" ></i></a>
                                    </td>
                                 </tr>
                              @endforeach
                        @endforeach
                        ...
                        <div class="form-group text-center">
                           <button type="submit" id="submitBtn" class="btn btn-success"><i class="fe fe-dollar"></i> Update Now</button>
                        </div>
                     </div>
                  </div>
               </form>
                  ...
                  <div class="container">
                     <div class="row" id="search-results">
                        @foreach ($products as $product)
                           <div class="col-md-4 col-sm col-xl-4">
                              <div class="product-list-box card p-2">
                                 <a href="javascript:void(0);"   onclick="__get_product({{ $product->id }})">
                                 <img src="{{ $product->product_image[0]->image }}" class="img-fluid" alt="work-thumbnail" style="height: 150px; width: 150px; ">
                                 </a>
                                 <div class="detail">
...
function fetchAllProducts(){
   $.ajax({
      ...
      data: { search: '' }, // Pass an empty search string
      success: function(response) {
         if (response.success) {
...
function displayProducts(products) {
   ...
   products_container.empty(); // Clear previous products
   $.each(products, function(index, product) {
      ...
      products_container.append(
            `<div class="col-md-4 col-sm col-xl-4">
               <div class="product-list-box card p-2">
                  <a href="javascript:void(0);" onclick="__get_product(${product.id})">
                        <img src="${imageUrl}" class="img-fluid" alt="work-thumbnail" style="height: 150px; width: 150px;">
                  </a>
                  <div class="detail">
